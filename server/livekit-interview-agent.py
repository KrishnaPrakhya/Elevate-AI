...
=============================
Real-time voice AI interviewer using LiveKit Agents (free tier).
...
class InterviewAgent(agents.Agent):
    """AI Interviewer that conducts voice/video mock interviews."""
    ...
    async def entrypoint(self, job: JobContext):
        ...
        logger.info(f"Starting interview agent for {self.role} - {self.level}")
    ...
    async def _wait_for_participant(self, room: rtc.Room, timeout: float = 30.0):
        ...
        while len(room.remote_participants) == 0:
            if asyncio.get_event_loop().time() - start_time > timeout:
                await self._say("It seems no one joined. Ending the session. Good luck!")
    ...
    async def _run_interview(self, room: rtc.Room, ctx: rtc.ChatContext):
        ...
        for i, q in enumerate(self.questions):
            ...
            # Wait for answer (voice pipeline handles this)
